...
public class PlyaerController : MonoBehaviour
{
    ...
    public void ModifyShoulderOffset(Vector3 v3)
    {
        ...
        {
            // 尝试获取Transposer组件（适用于Simple Follow、Follow等）
            var transposer = _cinemachineVirtualCamera.GetCinemachineComponent<Cinemachine.CinemachineTransposer>();

            if (transposer != null)
            {
                // 修改Shoulder Offset - 对于Transposer使用FollowOffset
                transposer.m_FollowOffset = v3;
            }
            else
            {
                // 尝试获取3rd Person Follow组件
                Cinemachine3rdPersonFollow thirdPersonFollow = _cinemachineVirtualCamera.GetCinemachineComponent<Cinemachine.Cinemachine3rdPersonFollow>();

                if (thirdPersonFollow != null)
                {
                    thirdPersonFollow.ShoulderOffset = v3;
                }
                else
                {
                    // 检查其他可能的Body组件类型
                    Debug.LogWarning("CinemachineTransposer or Cinemachine3rdPersonFollow component not found on virtual camera");
                }
            }
        }
    }
